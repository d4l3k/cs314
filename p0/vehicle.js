	...
	initScene = function() {
		...
		scene.addEventListener(
			...
			function() {
				...
				if ( input && vehicle ) {
					...
					if ( input.power === true ) {
						vehicle.applyEngineForce( 3000 ); // CHANGE: the force applied from the engine.
		...
		var number_of_items = 200; // CHANGE: the number of boxes in the simulation
		for ( i = 0; i < number_of_items; i++ ) {
			var size = Math.round( Math.random() * 2 + .5); // Some functions only like integers
